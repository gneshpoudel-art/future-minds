...
router.post('/', authMiddleware, validateCreate, async (req, res) => {
    ...
    try {
        const { branch_name, address, phone, map_link, display_order } = req.body;
        const result = await db.run(
            'INSERT INTO branches (branch_name, address, phone, map_link, display_order) VALUES (?,?,?,?,?)',
            [branch_name, address || '', phone || '', map_link || '', parseInt(display_order) || 0]
        );
...
router.put('/:id', authMiddleware, validateUpdate, async (req, res) => {
    ...
    try {
        const { branch_name, address, phone, map_link, display_order } = req.body;
        await db.run(
            'UPDATE branches SET branch_name=COALESCE(?,branch_name), address=COALESCE(?,address), phone=COALESCE(?,phone), map_link=COALESCE(?,map_link), display_order=COALESCE(?,display_order) WHERE id=?',
            [branch_name || null, address || null, phone || null, map_link || null, display_order ? parseInt(display_order) : null, req.params.id]
        );
